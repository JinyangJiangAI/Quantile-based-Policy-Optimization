...
class QMemory(object):
    ...
    def store_transition(self, state, action, reward, state_, is_terminal):
        ...
        self.states_.add(state_)
        self.rewards.add(reward)
        self.is_terminals.add(is_terminal)

    def get_transition(self, batch_size):
        idx = np.random.randint(0, min(self.memory_size, self.actions.idx), size=batch_size)
        batch_actions = torch.from_numpy(np.array([[self.actions.get(i)] for i in idx if self.actions.get(i) is not None])).long()
        batch_states = torch.from_numpy(np.array([self.states.get(i) for i in idx if self.states.get(i) is not None])).float()
        batch_states_ = torch.from_numpy(np.array([self.states_.get(i) for i in idx if self.states_.get(i) is not None])).float()
        batch_rewards = torch.from_numpy(np.array([[self.rewards.get(i)] for i in idx if self.rewards.get(i) is not None])).float()
        batch_is_terminals = torch.from_numpy(np.array([self.is_terminals.get(i) for i in idx if self.is_terminals.get(i) is not None])).float()
